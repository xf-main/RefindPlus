...
**/
VOID BdsBuildLegacyDevNameString (
    ...
    switch (Index) {
        case 1:  Fmt = L"Primary Master %s";   break; // Primary Master
        case 2:  Fmt = L"Primary Slave %s";    break; // Primary Slave
        case 3:  Fmt = L"Secondary Master %s"; break; // Secondary Master
        case 4:  Fmt = L"Secondary Slave %s";  break; // Secondary Slave
        default: Fmt = L"%s";                  break;
    } // switch
    ...
    if (Index >= 5 && Index <= 16 &&
        (CurBBSEntry->DeviceType == BBS_HARDDISK || CurBBSEntry->DeviceType == BBS_CDROM)
    ) {
        Fmt = L"%s %d";
...
**/
BOOLEAN BdsFindLegacyBootOptionByDevTypeAndName (
    IN  UINT16 *BootOrder,
...
EFI_STATUS BdsCreateLegacyBootOption (
    ...
    *((UINT16 *) Ptr) = (UINT16) Index;
    ...
    Status = REFIT_CALL_5_WRAPPER(
        ...
        &gEfiGlobalVariableGuid, VAR_FLAG,
        BufferSize, Buffer
    );
...
**/
VOID GroupMultipleLegacyBootOption4SameType (
    UINT16 *BootOption,
...
**/
EFI_STATUS EfiLibDeleteVariable (
    IN CHAR16   *VarName,
    IN EFI_GUID *VarGuid
)
{
...
**/
EFI_STATUS BdsDeleteBootOption (
